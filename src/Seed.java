import java.awt.*;

public class Seed extends ToolBlockGeneric{

    private Color seedColor;

    public Seed(Graphics g, int _xpos, int _ypos, int _w, int _h) {
        xPos = _xpos;
        yPos = _ypos;
        Width = _w;
        Height = _h;

        seedColor = new Color(0, 255, 0);
    }

    public boolean playerIsTouching(int playerX, int playerY, int playerWidth, int playerHeight) {
        if (playerX + playerWidth > this.xPos && playerX < this.xPos + 150 && playerY + playerHeight > this.yPos && playerY < this.yPos + 150) {
            Player.setFarmingMode(1);
            return true;
        }
        return false;
    }

    public void setSeedColor(int r, int g, int b) {
        seedColor = new Color(r, g, b);
    }

    public Color getSeedColor() {
        return seedColor;
    }

}
